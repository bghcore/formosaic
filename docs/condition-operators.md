...
// Show an error message when "restricted" is in the selection
{
  when: { field: "permissions", operator: "arrayNotContains", value: "restricted" },
  then: { hidden: true },
  else: { hidden: false },
}

// Make a field required only when exactly one item is selected
{
  when: { field: "items", operator: "arrayLengthEquals", value: 1 },
  then: { required: true },
}
```
