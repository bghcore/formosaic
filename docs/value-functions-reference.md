...
This document covers value functions in `dynamic-react-business-forms` -- imperative, named functions that compute or set field values at specific lifecycle moments. It also covers how value functions differ from computed values (`computedValue`).
...
```typescript
import { registerValueFunctions, ValueFunction } from "@bghcore/dynamic-forms-core";
