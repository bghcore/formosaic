...
The core package (`@bghcore/dynamic-forms-core`) provides an opt-in analytics system for tracking form lifecycle events. When analytics callbacks are provided in form settings, events such as field focus, blur, change, validation errors, form submission, abandonment, wizard step navigation, and rule evaluation are automatically tracked with zero configuration beyond supplying the callbacks.
...
```tsx
import { useBeforeUnload, useFormAnalytics } from "@bghcore/dynamic-forms-core";
...
Defined in `packages/core/src/hooks/useFormAnalytics.ts`. Exported from `@bghcore/dynamic-forms-core`.

```typescript
import { useFormAnalytics, IFormAnalytics } from "@bghcore/dynamic-forms-core";
...
import { IAnalyticsCallbacks } from "@bghcore/dynamic-forms-core";
