...
const MultiSelectSearch = (props: IFieldProps<{}>) => {
  ...
  if (readOnly) {
    return selectedValues.length > 0 ? (
      <ul className="ak-multi-select-search-readonly" data-field-type="MultiSelectSearch" data-field-state="readonly">
        {selectedValues.map((v, i) => (<li key={i}>{v}</li>))}
      </ul>
    ) : (<span className="ak-read-only-text">-</span>);
  }
  return (
    <div className="ak-multi-select-search" data-field-type="MultiSelectSearch" data-field-state={getFieldState({ error, required, readOnly })} data-testid={GetFieldDataTestId(fieldName, programName, entityType, entityId)}>
      ...
      <fieldset className="ak-multi-select-search__options" aria-invalid={!!error} aria-required={required}>
        ...
        {filteredOptions.map(option => {
          ...
          return (
            <label key={optVal} className="ak-multi-select-search__option">
