  ...
  return (
    ...
    >
      <input
        ref={inputRef}
        type="file"
        className="ak-file-upload__input"
        multiple={multiple}
        accept={accept}
        onChange={onChange}
        aria-invalid={!!error}
      ...
      <button
        type="button"
        className="ak-file-upload__button"
        onClick={() => inputRef.current?.click()}
      >
        {fileNames ? "Change file" : "Choose file"}
      </button>
      {fileNames && (
        <span className="ak-file-upload__filenames">{fileNames}</span>
      )}
      {sizeError && (
        <span className="ak-file-upload__size-error" role="alert">{sizeError}</span>
      )}
    </div>
  );
};
