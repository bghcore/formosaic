...
const RadioGroup = (props: IFieldProps<{}>) => {
  ...
  return (
    ...
    >
      {options?.map(option => (
        <label key={String(option.value)} className="ak-radio-group__option">
          <input
            type="radio"
            className="ak-radio-group__input"
            name={fieldName}
          ...
          />
          <span className="ak-radio-group__label">{option.label}</span>
        </label>
      ))}
    </div>
  );
