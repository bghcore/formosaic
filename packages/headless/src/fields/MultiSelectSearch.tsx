...
const MultiSelectSearch = (props: IFieldProps<{}>) => {
  ...
  if (readOnly) {
    return selectedValues.length > 0 ? (
      <ul
        className="df-multi-select-search-readonly"
  ...
  return (
    ...
    >
      <input
        type="search"
        className="df-multi-select-search__input"
        placeholder="Search..."
        value={searchTerm}
        onChange={e => setSearchTerm(e.target.value)}
        aria-label={`Search options for ${fieldName}`}
      />
      <fieldset className="df-multi-select-search__options" aria-invalid={!!error} aria-required={required}>
        <legend className="df-sr-only">Options</legend>
        {filteredOptions.map(option => {
          const optVal = String(option.value);
          const isChecked = selectedValues.includes(optVal);
          return (
            <label key={optVal} className="df-multi-select-search__option">
              <input
                type="checkbox"
                checked={isChecked}
                disabled={option.disabled}
                onChange={e => onCheckChange(optVal, e.target.checked)}
              />
